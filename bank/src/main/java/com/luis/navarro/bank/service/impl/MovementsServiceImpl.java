...
public class MovementsServiceImpl implements MovementsService {
	...
	public ResponseEntity<Response> save(MovementsDTO request) {
		 ...
		 * */
		if (Boolean.FALSE.equals(accountSearch.isEmpty()) && Boolean.FALSE.equals(request.getMovementValue().signum() != -1)
	...
	public ResponseEntity<Response> generateReportsBetweenDate(String accountNumber, String startDate, String endDate) {
		 try {
			...
			List<Movements> movementsList = movementsRepository.findAllMovementsByAccountNumberAndBetweenMovementDate(accountNumber, startDateConvert, endDateConvert);
			movementsList.stream().forEach(info -> {
				Map<String, Object> response = new LinkedHashMap<>();
				var fullName = info.getAccount().getCustomer().getNames().concat(" ").concat(info.getAccount().getCustomer().getSurnames());
				response.put("Fecha", new SimpleDateFormat("dd/MM/yyyy").format(info.getMovementDate()));
				response.put("Cliente", fullName != null && StringUtils.hasText(fullName) ? fullName : "");
				response.put("Numero Cuenta", info.getAccount().getAccountNumber());
				response.put("Tipo", info.getAccount().getAccountType());
				response.put("Saldo inicial", info.getBalance());
				response.put("Estado", info.getAccount().getAccountState());
				response.put("Movimiento", info.getMovementValue());
				BigDecimal amount = BigDecimal.ZERO;
				if (info.getMovementValue().signum() != -1) {
					amount = info.getBalance().subtract(info.getMovementValue());
				} else {
					amount = info.getBalance().add(info.getMovementValue());
				}
				response.put("Saldo Disponible", amount);
				responseList.add(response);
			});
			return ResponseFactory.ok(responseList);
